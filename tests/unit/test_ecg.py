...
class ECGModule(unittest.TestCase):
	...
	def assertSequenceAlmostEqual(self, list1, list2, tol=None):
		self.assertEqual(len(list1), len(list2))
		for i in range(len(list1)):
			self.assertAlmostEqual(list1[i], list2[i], delta=tol)
	...
	def aux_test_RPeak_detector(self, str_detector, **args):
		...
		r_peaks = eval('ecg.' + str_detector)(self.ecg_filtered, **args)['rpeaks']
		# 2. Correct Rpeaks to R position
		r_peaks = ecg.correct_rpeaks(self.ecg_filtered, r_peaks)['rpeaks']
		# 2. Compare new rpeaks with those previously defined
		self.assertSequenceAlmostEqual(r_peaks, self.r_peaks, tol=5)
	...
	def test_ecg_incorrect_sampling_frequencies_value_error(self):
		...
		for sampling_frequency in fs:
			with self.subTest(sampling_frequency=sampling_frequency):
				self.assertRaises(ValueError, ecg.ecg, self.ecg_signal, sampling_rate=sampling_frequency, show=False,
								  interactive=False)
	...
	# def test_ecg_correct_filtered_signal(self):
	#
	# 	ecg_info = ecg.ecg(self.ecg_signal, show=False)
	# 	self.assertSequenceAlmostEqual(ecg_info["filtered"], self.ecg_filtered)
